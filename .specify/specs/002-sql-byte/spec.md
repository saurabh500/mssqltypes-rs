...
4. *(Deferred — SqlInt16 not yet implemented)* **Given** `SqlByte(100)`, **When** converted to `SqlInt16`, **Then** returns `SqlInt16(100)`
...
- **FR-007**: MUST provide conversions to/from existing types (`From<u8>`, `SqlBoolean`); wider integer conversions (SqlInt16, SqlInt32, SqlInt64) deferred to their respective implementations
