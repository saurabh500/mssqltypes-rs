...
- [X] T001 Create empty `src/sql_byte.rs` with `use crate::error::SqlTypeError;` and `use crate::sql_boolean::SqlBoolean;` imports, plus a `#[cfg(test)] mod tests {}` block.
- [X] T002 Update `src/lib.rs` — add `pub mod sql_byte;` and `pub use sql_byte::SqlByte;`.
...
- [X] T003 [US1] Write constant and construction tests in `src/sql_byte.rs` `#[cfg(test)] mod tests`:
...
- [X] T004 [US1] Write Copy/Clone/Debug tests in `src/sql_byte.rs`:
...
- [X] T005 [US1] Implement `SqlByte` struct, constants, and constructors in `src/sql_byte.rs`:
...
- [X] T006 [US1] Implement `From<u8> for SqlByte` in `src/sql_byte.rs`.
...
- [X] T007 [US2] Write addition tests in `src/sql_byte.rs`:
...
- [X] T008 [US2] Write subtraction tests in `src/sql_byte.rs`:
...
- [X] T009 [US2] Write multiplication tests in `src/sql_byte.rs`:
...
- [X] T010 [US2] Write division and remainder tests in `src/sql_byte.rs`:
...
- [X] T011 [US2] Implement `checked_add`, `checked_sub`, `checked_mul` in `src/sql_byte.rs`:
...
- [X] T012 [US2] Implement `checked_div` and `checked_rem` in `src/sql_byte.rs`:
  - Check divisor == 0 → `Err(DivideByZero)`
  - NULL propagation: if either is NULL, return `Ok(SqlByte::NULL)`
  - No overflow possible for div/rem

- [X] T013 [US2] Implement `Add`, `Sub`, `Mul`, `Div`, `Rem` operator traits in `src/sql_byte.rs`:
...
- [X] T014 [US3] Write bitwise operation tests in `src/sql_byte.rs`:
...
- [X] T015 [US3] Implement `ones_complement`, `BitAnd`, `BitOr`, `BitXor`, `Not` in `src/sql_byte.rs`:
...
- [X] T016 [US4] Write SQL comparison tests in `src/sql_byte.rs`:
...
- [X] T017 [US4] Write PartialEq, Eq, Hash, Ord tests in `src/sql_byte.rs`:
...
- [X] T018 [US4] Write Display and FromStr tests in `src/sql_byte.rs`:
...
- [X] T019 [US4] Write conversion tests in `src/sql_byte.rs`:
...
- [X] T020 [US4] Implement SQL comparison methods in `src/sql_byte.rs`:
...
- [X] T021 [US4] Implement `PartialEq`, `Eq`, `Hash`, `PartialOrd`, `Ord` in `src/sql_byte.rs`:
...
- [X] T022 [US4] Implement `Display` and `FromStr` in `src/sql_byte.rs`:
...
- [X] T023 [US4] Implement `to_sql_boolean()` and `From<SqlBoolean> for SqlByte` in `src/sql_byte.rs`:
...
- [X] T024 Run `cargo fmt --check` and fix any formatting issues in `src/sql_byte.rs`.
- [X] T025 Run `cargo clippy -- -D warnings` and fix all warnings.
- [X] T026 Run `cargo test` — verify all tests pass.
- [X] T027 Run coverage tool (`cargo tarpaulin` or `llvm-cov`) — verify ≥95% coverage for `src/sql_byte.rs`.
- [X] T028 Review public API surface — ensure only spec'd methods/traits are `pub`. No leaking of internals.
